...
let user: userType = { name: "Alice", isAdmin: true };
...
function getProductNamesAndPrices(
  ...
  // .pop()을 고민했으나 튜플을 직접 수정하여 불변성을 해쳐 안전하지 않는 방법이라 기각.
...
function getAvailableProducts(
  ...
  return products.filter((productInfo) => productInfo[2]);
}
...
// proudcts 타입정의  필요
type products2Type = { name: string; price: number; category?: string }[];
const products2: products2Type = [
